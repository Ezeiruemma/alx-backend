...
Create a queue with Kue Create an object containing the Job data with the following format:
...
After this array created:
 ...
 Create a new job to the queue push_notification_code_2 with the current object If there is no error, log to the console Notification job created: JOB_ID
...
Create a function sendNotification that takes 4 arguments: phoneNumber, message, job, and done:
 ...
 Log to the console Sending notification to PHONE_NUMBER, with message: MESSAGE Create a queue with Kue that will proceed job of the queue push_notification_code_2 with two jobs at a time.
Requirements:

 You only need one Redis server to execute the program
 You will need to have two node processes to run each script at the same time You muse use Kue to set up the queue
...
Requirements:
 ...
 Make sure the format returned by the web application is always JSON and not text Repo:
